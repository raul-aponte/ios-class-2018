...
class BaseClient {
    ...
    func request(post endpoint: String, params: [String: Any]? = nil) -> Observable<JSON> {
        return getObservable(method: .post, endpoint: endpoint, params: params, encoding: JSONEncoding.default)
    }

    func request(put endpoint: String, params: [String: Any]? = nil) -> Observable<JSON> {
        return getObservable(method: .put, endpoint: endpoint, params: params, encoding: JSONEncoding.default)
    }
    ...
    private func getObservable(
        ...
        return Observable<JSON>.create({ (observer) -> Disposable in
            ...
            let dataRequest = Alamofire.request(url, method: method, parameters: params, encoding: encoding, headers: self.headers)
            let request = dataRequest.responseJSON(completionHandler: { (response) in
